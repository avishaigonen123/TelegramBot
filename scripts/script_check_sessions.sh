...
for dir in $(find $START_DIR -type d -name 'bot_*' -not -path '*/\.*'); do
    echo "Entering directory: $dir"
    cd "$dir"

    # Check if the check_sessions.py script exists in the directory
    if [ -f "check_sessions.py" ]; then
        echo "Running check_sessions.py in directory: $dir"
        python3 check_sessions.py
    else
        echo "check_sessions.py not found in the directory of $dir"
    fi
